...
def upgrade() -> None:
    ...
    # Step 2 + 3: Add unique constraint + index in batch mode (SQLite compat)
    with op.batch_alter_table("role_tool_permissions") as batch_op:
...
def downgrade() -> None:
    with op.batch_alter_table("role_tool_permissions") as batch_op:
        batch_op.drop_index("ix_role_tool_permission_role_tool")
        batch_op.drop_constraint("uq_role_tool_permission_role_tool", type_="unique")
