...
import { Badge, Pagination, Select, Spinner, ToggleSwitch } from "@/components/ui";
import { listAuditLogs, listSecurityEvents, listToolExecutions } from "@/services/admin";
...
function UserCell({
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: admin-only inline click (follow-up issue: convert to <button>)
...
export default function AdminLogsPage() {
  ...
  const handleTabChange = (tabKey: LogTab) => {
    setTab(tabKey);
    setOffset(0);
    setToolFilter("all");
  };
      ...
      <div className="card p-4">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          ...
          {tab === "tool-executions" && (
            <Select
              options={[
                { value: "all", label: t("admin.all_tools") },
                { value: "ping", label: t("tools.ping.name") },
                { value: "traceroute", label: t("tools.traceroute.name") },
                { value: "dns_lookup", label: t("tools.dns.name") },
              ...
              ariaLabel={t("admin.filter_by_tool")}
            />
          )}
          ...
          <div className="flex items-center gap-1 ms-auto">
            ...
            <Select
              ...
              ariaLabel={t("admin.rows_per_page")}
            />
          </div>
        </div>
          ...
          <>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--color-border)]">
                      <th className="px-3 py-2 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase whitespace-nowrap">
                        {t("admin.date_time")}
                      </th>
                      <th className="px-3 py-2 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                        {t("admin.username")}
                      </th>
                      <th className="px-3 py-2 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                        {t("admin.ip")}
                      ...
                      <th className="px-3 py-2 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase hidden md:table-cell">
                        {t("admin.query")}
                      </th>
                      <th className="px-3 py-2 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase hidden md:table-cell">
                        {t("admin.result")}
                      </th>
                      <th className="px-3 py-2 text-end text-xs font-semibold text-[var(--color-text-secondary)] uppercase hidden lg:table-cell">
                        {t("common.duration")}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {toolLogs.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="px-3 py-8 text-center text-[var(--color-text-secondary)]">
                          {t("admin.no_logs_found")}
                        </td>
                      </tr>
                    ) : (
                      toolLogs.map((log) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-3 py-2 text-[var(--color-text-secondary)] text-xs whitespace-nowrap">
                            ...
                            <td
                              className={`px-3 py-2 font-medium text-[var(--color-text)] ${isSuccess ? "max-md:text-success-600 max-md:dark:text-success-500" : isPartial ? "max-md:text-warning-600 max-md:dark:text-warning-500" : "max-md:text-error-600 max-md:dark:text-error-500"}`}
                            >
                              {toolDisplayName(log.tool_name)}
                            </td>
                            <td
                              className="px-3 py-2 text-[var(--color-text)] text-xs font-mono max-w-[200px] truncate hidden md:table-cell"
                              title={log.query}
                            >
                              {log.query || "—"}
                            </td>
                            ...
                            <td className="px-3 py-2 text-end font-mono text-[var(--color-text)] text-xs hidden lg:table-cell">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {securityLogs.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-3 py-8 text-center text-[var(--color-text-secondary)]">
                          {t("admin.no_events_found")}
                        </td>
                      </tr>
                    ) : (
                      securityLogs.map((log) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 font-mono text-[var(--color-text)] text-xs">
                            {log.source_ip || log.ip_address || "-"}
                          </td>
                          <td className="px-3 py-2">
                            <Badge variant={getSecurityBadgeVariant(log.event_type)}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {auditLogs.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-3 py-8 text-center text-[var(--color-text-secondary)]">
                          {t("admin.no_audit_entries_found")}
                        </td>
                      </tr>
                    ) : (
                      auditLogs.map((log) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2 text-[var(--color-text-secondary)] text-xs hidden md:table-cell">
                              {detail || "—"}
                            </td>
                          </tr>
                        );
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
              <span className="text-xs text-[var(--color-text-secondary)]">
                {t("admin.entries_count", { count: pagination.total })}
              </span>
