...
export default function SslViewerPage() {
  ...
  const isWarning = (issue: string) => issue === t("tools.ssl.issue_no_cn") || issue === t("tools.ssl.issue_empty_subject") || issue === t("tools.ssl.issue_ocsp_unknown");
  ...
  return (
    <PageLayout>
      ...
      >
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-medium text-[var(--color-text-secondary)]">{t("tools.ssl.sni_label")} {t("tools.ssl.sni_optional_hint")}</span>
          <TextInput type="text" placeholder={t("tools.ssl.sni_desc")} value={form.sni} onChange={(e) => update("sni", e.target.value)} />
        </label>
      </ToolForm>
          ...
          <div className="space-y-3">
            <div className="card p-3 flex flex-wrap items-center gap-3">
              <div className="flex flex-wrap gap-2">
                <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.tls_version_label")}</span>
                <span className="font-mono text-sm text-[var(--color-text)]">{sslResult.tls_version}</span>
              </div>
              <div className="flex flex-wrap gap-2">
                <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.cipher_label")}</span>
                ...
                return {
                  ...
                  trigger: (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {issues.map((iss) => {
                        const variant = iss === t("tools.ssl.issue_trusted") ? "success" : isWarning(iss) ? "warning" : "error";
                        return <Badge key={iss} variant={variant} className="text-[10px]">{iss}</Badge>;
                      })}
                      {issues.length === 0 && <Badge variant="success" className="text-[10px]">{t("tools.ssl.valid")}</Badge>}
                    </div>
                  ),
                  content: (
                    <div className="space-y-2">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.subject_label")}</span>
                        ...
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.issuer_label")}</span>
                          <p className={`font-mono text-xs ${cert.is_self_signed || cert.is_untrusted || cert.missing_issuer ? errColor : okColor}`}>
                            ...
                            {cert.missing_issuer && cert.missing_issuer_name && (
                              <span className="text-error-600 dark:text-error-500">{t("tools.ssl.root_not_found")} {cert.missing_issuer_name}</span>
                            )}
                          </p>
                        </div>
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.valid_from_label")}</span>
                          <p className={`font-mono text-xs ${cert.is_expired ? errColor : okColor}`}>{formatDate(cert.valid_from)}</p>
                        </div>
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.valid_until_label")}</span>
                          <p className={`font-mono text-xs ${cert.is_expired ? errColor : okColor}`}>{formatDate(cert.valid_until)}</p>
                        </div>
                        {cert.revocation_status && (
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.revocation_label")}</span>
                        ...
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.key_algorithm_label")}</span>
                          <p className={`font-mono text-xs ${cert.is_weak_key ? errColor : okColor}`}>{cert.key_algorithm} {cert.key_size} bits</p>
                        </div>
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.signature_label")}</span>
                          <p className="font-mono text-xs text-[var(--color-text)]">{cert.signature_algorithm}</p>
                        </div>
                      </div>
                      {cert.sans.length > 0 && (
                        <div>
                          <span className={`text-xs font-semibold ${cert.name_mismatch ? errColor : 'text-[var(--color-text-secondary)]'}`}>{t("tools.ssl.san_label")}</span>
                      ...
                      {cert.extended_key_usage.length > 0 && (
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.ext_key_usage_label")}</span>
                          <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      <div className="border-t border-[var(--color-border)] pt-2 mt-2">
                        <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase mb-2">{t("tools.ssl.extensions_label")}</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div>
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.serial_number_label")}</span>
                            <p className="font-mono text-xs text-[var(--color-text)]">{cert.serial_number}</p>
                          </div>
                          <div>
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.basic_constraints_label")}</span>
                            <p className="font-mono text-xs text-[var(--color-text)]">
                              {cert.is_ca ? t("tools.ssl.ca_yes") : t("tools.ssl.ca_no")}{cert.bc_path_length != null ? `${t("tools.ssl.pathlen_prefix")}${cert.bc_path_length}` : ""}
                            </p>
                          </div>
                          {cert.ski && (
                          <div>
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.ski_label")}</span>
                            <p className="font-mono text-[10px] text-[var(--color-text)] break-all">{cert.ski}</p>
                          </div>
                          )}
                          {cert.aki && (
                          <div>
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.aki_label")}</span>
                            <p className="font-mono text-[10px] text-[var(--color-text)] break-all">{cert.aki}</p>
                          </div>
                          )}
                          {cert.key_usage.length > 0 && (
                          <div className="sm:col-span-2">
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.key_usage_label")}</span>
                            <div className="flex flex-wrap gap-1 mt-1">
                          ...
                          <div className="sm:col-span-2">
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.aia_label")}</span>
                          ...
                          <div className="sm:col-span-2">
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.crl_label")}</span>
                          ...
                          <div className="sm:col-span-2">
                            <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.cert_policies_label")}</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.sha256_fingerprint_label")}</span>
                          <p className="font-mono text-[10px] text-[var(--color-text)] break-all">{cert.fingerprint_sha256}</p>
                        </div>
                        <div>
                          <span className="text-xs font-semibold text-[var(--color-text-secondary)]">{t("tools.ssl.sha1_fingerprint_label")}</span>
                          <p className="font-mono text-[10px] text-[var(--color-text)] break-all">{cert.fingerprint_sha1}</p>
                        </div>
                      </div>
                      <details className="mt-2">
                        <summary className="text-xs font-semibold text-[var(--color-text-secondary)] cursor-pointer hover:text-[var(--color-text)]">{t("tools.ssl.raw_pem_label")}</summary>
