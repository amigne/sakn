...
import type { DnsRecord, DnsResult } from "@/types/tool";
  ...
  return (
    <label className="flex flex-col gap-1 relative">
      ...
      <input
        ...
        value={displayValue}
        onChange={(e) => {
          onChange(e.target.value || "__system__");
          setOpen(true);
          setFocusedIdx(-1);
        }}
        onFocus={() => {
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter") {
            ...
            if (!displayValue || filtered.length === 0) onEnter();
...
export default function DnsLookupPage() {
  ...
  const [dnsServers, setDnsServers] = useState<DnsServerOption[]>([{ value: "__system__", label: "" }]);
  ...
  const toggleRecordType = (rt: string, checked: boolean | "indeterminate") => {
    ...
    const newTypes = checked ? [...form.record_types, rt] : form.record_types.filter((t) => t !== rt);
  ...
  const copyResults = () => {
    ...
    if (dnsResult.cname_chain) {
      lines.push("\n;; CNAME Chain:");
      // biome-ignore lint/suspicious/useIterableCallbackReturn: side-effect push, no return needed
    ...
    <PageLayout>
      ...
      >
        ...
        <label className="flex flex-col gap-1">
          ...
          <Checkbox
            checked={form.recursive_cname}
            onChange={(v) => update("recursive_cname", v === true)}
            label={t("tools.dns.cname_desc")}
          />
        </label>
        <div className="sm:col-span-2 lg:col-span-4">
          <span className="text-xs font-medium text-[var(--color-text-secondary)]">
            {t("tools.dns.record_type_label")}
          </span>
      ...
      <ToolOutput status={status} error={error} onCopy={dnsResult ? copyResults : undefined}>
          ...
          <div className="space-y-3">
            {dnsResult.dnssec_ad_flag && (
              <div className="card p-2 bg-success-50 dark:bg-success-50/10 border-success-500">
                <span className="text-xs font-mono text-success-600 dark:text-success-500">
                  {t("tools.dns.dnssec_ad_flag")}
                </span>
              </div>
            )}
            ...
            <div className="card p-3">
              ...
              <div className="space-y-2">
                {sortedOwnEntries().map(([type, recs]) => (
                  <div key={type}>
                    <h4 className="text-[11px] font-semibold text-[var(--color-text-secondary)] uppercase mb-1">
                      {t("tools.dns.records_section", { type })}
                    </h4>
            ...
            {dnsResult.cname_records &&
              Object.entries(dnsResult.cname_records).map(([hop, hopRecords]) => {
                  ...
                  <div key={hop} className="card p-3 border-s-2 border-primary-400">
                    <h3 className="text-xs font-semibold text-primary-600 dark:text-primary-400 uppercase mb-2">
                      ...
                      <span className="text-[var(--color-text-secondary)] font-normal normal-case">
                        {t("tools.dns.cname_chain_suffix")}
                      </span>
                    </h3>
            ...
            {dnsResult.authority && Object.keys(dnsResult.authority).length > 0 && (
              <div className="card p-3 border-s-2 border-warning-500">
                <h3 className="text-xs font-semibold text-warning-600 dark:text-warning-500 uppercase mb-2">
                  {t("tools.dns.authority_section")}
                </h3>
                <div className="font-mono text-sm text-[var(--color-text)] space-y-0.5">
            ...
            {dnsResult.additional && Object.keys(dnsResult.additional).length > 0 && (
              <div className="card p-3 border-s-2 border-info-500">
                <h3 className="text-xs font-semibold text-info-600 dark:text-info-500 uppercase mb-2">
                  {t("tools.dns.additional_section")}
                </h3>
                <div className="font-mono text-sm text-[var(--color-text)] space-y-0.5">
