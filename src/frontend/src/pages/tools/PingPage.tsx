...
import { Badge, ProgressBar, TextInput, ToggleSwitch } from "@/components/ui";
  ...
  return (
    <PageLayout>
      <ToolForm
          ...
          <>
            ...
            <label className="flex flex-col gap-1">
              ...
              <TextInput
                ...
                max={63}
                value={form.dscp}
                onChange={(e) => update("dscp", Number(e.target.value))}
                error={errors.dscp}
              />
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-xs font-medium text-[var(--color-text-secondary)]">
                {t("tools.ping.max_duration_label")}
              </span>
              <TextInput
                type="number"
                min={1}
        ...
        onStop={cancel}
        startLabel={
          (form.count ?? 0) > 0 ? t("tools.ping.execute_n_pings", { count: form.count }) : t("tools.ping.execute_label")
        }
        outputControls={
      ...
      >
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-medium text-[var(--color-text-secondary)]">
            {t("tools.ping.count_label")} (0 = unlimited)
          </span>
          <TextInput
            type="number"
            min={0}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-medium text-[var(--color-text-secondary)]">
            {t("tools.ping.packet_size_label")}
          </span>
          <TextInput
            type="number"
            min={8}
            ...
            onChange={(e) => update("packet_size", Number(e.target.value))}
      ...
      >
        {status !== "idle" && (
          <>
            {isRunning && (
              <ProgressBar value={pingResults.length} max={runCount || pingResults.length + 5} className="mb-3" />
            )}
            {displayMode === "table" ? (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--color-border)]">
                    ...
                    >
                      {t("tools.ping.table_header_seq")}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase"
                    >
                      {t("tools.ping.table_header_status")}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase"
                    >
                      {t("tools.ping.table_header_rtt")}
                    </th>
                    <th
                      scope="col"
                      className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase"
                    >
                      {t("tools.ping.table_header_ttl")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  {pingSummary && (
                    <>
                      {displayMode === "table" ? (
                        <>
                          <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase mb-1">
                            {t("tools.ping.section_packets")}
                          </h4>
                          <table className="w-full max-w-xs mb-3 text-sm">
                            <thead>
                              <tr className="border-b border-[var(--color-border)]">
                                <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                  {t("tools.ping.table_header_sent")}
                                </th>
                                <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                  {t("tools.ping.table_header_received")}
                                </th>
                                <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                  {t("tools.ping.table_header_lost")}
                                </th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr>
                                <td className="px-3 py-1 font-mono text-[var(--color-text)]">
                                  {pingSummary.transmitted}
                                </td>
                              ...
                              <tr>
                                ...
                                <td className="px-3 py-1 font-mono text-[var(--color-text-secondary)]">
                                  {(100 - pingSummary.loss_pct).toFixed(1)}%
                                </td>
                                <td className="px-3 py-1 font-mono text-[var(--color-text-secondary)]">
                      ...
                      {pingSummary.rtt_min_ms !== null ? (
                        displayMode === "table" ? (
                          <>
                            <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase mb-1 mt-3">
                              {t("tools.ping.section_rtt")}
                            </h4>
                            <table className="w-full max-w-xs text-sm">
                              <thead>
                                <tr className="border-b border-[var(--color-border)]">
                                  <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                    {t("tools.ping.table_header_min")}
                                  </th>
                                  <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                    {t("tools.ping.table_header_avg")}
                                  </th>
                                  <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                    {t("tools.ping.table_header_max")}
                                  </th>
                                  <th className="px-3 py-1 text-start text-xs font-semibold text-[var(--color-text-secondary)] uppercase">
                                    {t("tools.ping.table_header_stddev")}
                                  </th>
                                </tr>
                              </thead>
                              <tbody>
                                <tr>
                                  <td className="px-3 py-1 font-mono text-[var(--color-text)]">
                                    {pingSummary.rtt_min_ms} ms
                                  </td>
                                  <td className="px-3 py-1 font-mono text-[var(--color-text)]">
                                    {pingSummary.rtt_avg_ms} ms
                                  </td>
                                  <td className="px-3 py-1 font-mono text-[var(--color-text)]">
                                    {pingSummary.rtt_max_ms} ms
                                  </td>
                                  <td className="px-3 py-1 font-mono text-[var(--color-text)]">
                                    {pingSummary.rtt_mdev_ms} ms
                                  </td>
                                </tr>
                              </tbody>
                            </table>
