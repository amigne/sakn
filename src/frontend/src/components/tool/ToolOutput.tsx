...
export default function ToolOutput({
  status,
  emptyMessage = "Enter a target and click Start.",
  error,
  children,
  onCopy,
  viewToggle,
  ...
  return (
    <div className="card mt-4 p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-sm font-semibold text-[var(--color-text)]">Output</h2>
      ...
      {status === "idle" && !error && (
        <p className="py-8 text-center text-sm text-[var(--color-text-secondary)]">{emptyMessage}</p>
      )}
      ...
      {(status === "running" || status === "completed" || status === "stopped") && children}
    </div>
  );
}
